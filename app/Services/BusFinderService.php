...
class BusFinderService
{
    public function findBuses(BusFinderDTO $dto): array
    {
        ...
        foreach ($routeDirections as $direction) {
            ...
            $lastStop = $direction->routeStops()
                ->orderBy('stop_order', 'desc')
                ->first();

            $result[] = [
                'route' => sprintf(
                    'Автобус №%s в сторону %s',
